...
class Trainer(object):
        ...
        for epoch in range(start, self.total_epoch):
            ...
            for batch_x, batch_y in train_pbar:
                ...
                for _ in range(self.aft_seq_length):
                    ...
                    cur_seq = torch.cat((cur_seq.clone()[:,1:], pred_y.clone()[:,:1]), 1)
                    expected = torch.cat((batch_x.clone()[:,1:], batch_y.clone()[:,:1]), 1)
    ...
    def _predict(self, batch_x):
        ...
        for _ in range(self.aft_seq_length):
            ...
            cur_seq = torch.cat((cur_seq.clone()[:,1:], output.clone()), 1)
